...
export async function renderDin5008Document(subject: string, body: string, snapshot: Record<string, unknown>) {
  const company = get(snapshot, "company"); const debtor = get(snapshot, "debtor"); const address = get(debtor, "address"); const ledger = get(snapshot, "ledger"); const caseData = get(snapshot, "case"); const portalAccess = get(snapshot, "portalAccess");
