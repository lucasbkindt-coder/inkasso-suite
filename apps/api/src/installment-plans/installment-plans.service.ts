...
import { ActivityEventType, InstallmentPlanItemStatus, InstallmentPlanStatus, InstallmentRequestStatus, LedgerEntrySide, LedgerEntryStatus, LedgerEntryType, Prisma } from "@prisma/client";
...
export class InstallmentPlansService {
  ...
  async createFromRequest(requestId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.activity.recordStaffEvent(tx, createdByMembershipId, { tenantId, caseId: plan.caseId, partyId: plan.debtorPartyId, eventType: ActivityEventType.INSTALLMENT_PLAN_CREATED, description: "Ratenplan wurde erstellt.", metadata: { installmentPlanId: plan.id, installmentRequestId: request.id }, sourceEntityType: "InstallmentPlan", sourceEntityId: plan.id });
