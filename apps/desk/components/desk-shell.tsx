...
import { Headphones, Inbox, LayoutDashboard, LogOut, Mail, Moon, PhoneCall, PhoneMissed, Plus, Settings, ShieldAlert, Sun, UserRoundCheck } from "lucide-react";
...
import { SoftphoneBar } from "./softphone-bar";
...
const nav = [
  ...
  { label: "Anrufe", href: "/calls", icon: PhoneCall, view: "calls" },
  { label: "Verpasst", href: "/calls?missed=true", icon: PhoneMissed, view: "missed" },
  { label: "Meine Telefonie", href: "/settings/telephony", icon: Settings, view: "telephony-settings" },
] as const;
...
export function DeskShell({ children }: { children: React.ReactNode }) {
  ...
  const canReadTelephony = session?.permissions.includes("desk:telephony:read") ?? false;
  ...
  function active(view: (typeof nav)[number]["view"]) {
    ...
    if (view === "calls") return pathname === "/calls" && search.get("missed") !== "true";
    if (view === "missed") return pathname === "/calls" && search.get("missed") === "true";
    if (view === "telephony-settings") return pathname.startsWith("/settings/telephony");
  ...
  return <div className="min-h-screen bg-background lg:grid lg:grid-cols-[248px_1fr]">
    <aside className="border-b bg-[#071d2b] text-white lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:border-slate-800">
      ...
      <nav className="flex gap-1 overflow-x-auto p-3 lg:block lg:space-y-1 lg:overflow-visible" aria-label="Desk-Navigation">{nav.filter((item) => !["calls","missed","telephony-settings"].includes(item.view ?? "") || canReadTelephony).map((item) => { const Icon=item.icon; return <Link className={`flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium ${active(item.view) ? "bg-white/12 text-white" : "text-slate-400 hover:bg-white/8 hover:text-white"}`} href={item.href} key={item.href}><Icon className="size-4" />{item.label}</Link>; })}</nav>
    ...
    <div className="min-w-0">
      ...
      <main className="mx-auto w-full max-w-[1500px] p-4 pb-28 sm:p-6 sm:pb-28 lg:p-8 lg:pb-28">{children}</main>
    </div>
    <SoftphoneBar />
  </div>;
}
