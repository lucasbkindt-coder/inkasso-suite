    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between gap-5 px-5 sm:px-8">
        <Link aria-label="payveo Startseite" className="shrink-0 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#007FC5]" href="/">
          <Image alt="payveo" className="h-auto w-28 sm:w-32" priority src={payveoLogo} />
        </Link>
        ...
        <div className="hidden items-center gap-2 xl:flex">
          <Link className="px-3 py-2 text-sm font-semibold text-slate-700 transition hover:text-[#006da9] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#007FC5]" href="/portal/login">Mandantenportal</Link>
          <Link className="rounded-xl bg-[#007FC5] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#006da9] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#007FC5]" href="/portal/login">Schuldnerportal</Link>
        </div>
      ...
      {open ? <nav aria-label="Mobile Hauptnavigation" className="border-t border-slate-200 bg-white px-5 py-5 xl:hidden" id="public-mobile-navigation">
        <div className="mx-auto grid max-w-7xl gap-1">
          ...
          <div className="mt-4 grid gap-2 border-t border-slate-200 pt-4 sm:grid-cols-2">
            <Link className="rounded-xl border border-slate-300 px-4 py-3 text-center text-sm font-semibold text-slate-800" href="/portal/login" onClick={() => setOpen(false)}>Mandantenportal</Link>
            <Link className="rounded-xl bg-[#007FC5] px-4 py-3 text-center text-sm font-semibold text-white" href="/portal/login" onClick={() => setOpen(false)}>Schuldnerportal</Link>
          </div>
        </div>
      </nav> : null}
    </header>
