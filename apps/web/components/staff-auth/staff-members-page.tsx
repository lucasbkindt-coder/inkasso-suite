...
export function StaffMembersPage() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const [memberValues, roleValues] = await Promise.all([staffAdminApi.list(), staffAdminApi.roles()]);
      setMembers(memberValues); setRoles(roleValues); setError("");
...
function MemberDialog({ open, onOpenChange, onCreated, roles }: { open: boolean; onOpenChange: (value: boolean) => void; onCreated: () => void; roles: StaffRole[] }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/20 p-4"><form className="w-full max-w-lg space-y-4 rounded-2xl border bg-card p-6 shadow-xl" onSubmit={submit}><div className="flex justify-between"><h2 className="text-lg font-semibold">Mitarbeiter anlegen</h2><Button onClick={() => onOpenChange(false)} type="button" variant="ghost">Schließen</Button></div><label className="grid gap-1 text-sm font-medium">Name<input className="h-10 rounded-lg border bg-background px-3 font-normal" onChange={(event) => setName(event.target.value)} required value={name} /></label><label className="grid gap-1 text-sm font-medium">E-Mail<input className="h-10 rounded-lg border bg-background px-3 font-normal" onChange={(event) => setEmail(event.target.value)} required type="email" value={email} /></label><label className="grid gap-1 text-sm font-medium">Temporäres Passwort<input className="h-10 rounded-lg border bg-background px-3 font-normal" minLength={12} onChange={(event) => setPassword(event.target.value)} required type="password" value={password} /></label><fieldset className="grid gap-2"><legend className="text-sm font-medium">Rollen</legend>{roles.map((role) => <label className="flex items-center gap-2 text-sm" key={role.id}><input checked={roleIds.includes(role.id)} onChange={(event) => setRoleIds((current) => event.target.checked ? [...current, role.id] : current.filter((id) => id !== role.id))} type="checkbox" />{role.name}</label>)}</fieldset>{error ? <p className="text-sm text-destructive">{error}</p> : null}<Button disabled={pending} type="submit">{pending ? <><Loader2 className="size-4 animate-spin" /> Wird angelegt …</> : "Mitarbeiter anlegen"}</Button></form></div>;
}
